...
def display(query_data, active_filters, start_date, end_date):
    with stylable_container(
        ...
        fig1.add_trace(go.Scatter(
            ...
            yaxis='y2',
            text=[format_number(val) for val in df_top10['CPM']],  # Use short format (K, M)
        ...
        fig1.update_layout(
            ...
            yaxis2=dict(title='CPM', overlaying='y', side='right', showgrid=False), 
        ...
        try:
            ...
            # If the query fails, fallback to basic columns
                ...
                start_date=start_date, 
                end_date=end_date,
                    aggregations={"Clicks": "SUM", "Impression": "SUM"},
                group_by=["Region"]
            )
            ...
            # Convert numeric columns to float to avoid dtype errors
                ...
                effectiveness_df = effectiveness_df.sort_values('Clicks', ascending=False)
                
                # Display Effectiveness table using Material-UI style
                ...
                with mui.Paper(
                    ...
                    with mui.TableContainer(sx={"maxHeight": 400, "position": "relative"}):
                        with mui.Table(stickyHeader=True, size="small"):
                            ...
                            with mui.TableHead():
                                with mui.TableRow():
                                        headers = ["Region", "Brand", "Funnel", "Platform", "Format", "Audience", "Clicks", "VTR", "CTR"]
                                    for header in headers:
                                        mui.TableCell(
                                            header,
                                            sx={
                            ...
                            # Calculate totals for sticky footer
                                total_clicks = effectiveness_df['Clicks'].sum()
                                total_vtr = effectiveness_df['VTR'].mean()
                                total_ctr = effectiveness_df['CTR'].mean()
                            ...
                            with mui.TableFooter(
                                ...
                                with mui.TableRow():
                                    # TOTAL label (spans 6 columns for Region, Brand, Funnel, Platform, Format, Audience)
                                    mui.TableCell(
                                        "TOTAL",
                                        sx={
                                            "textAlign": "center",
                ...
                st.warning("No effectiveness data available for the selected filters.")
            ...
            try:
                efficiency_df = query_data(
                    ["Region", "Brand", "Funnel", "Platform", "Format", "Audience", "Cost", "Views", "Clicks", "Impression"], 
                    table, 
                    filters=active_filters, 
                    start_date=start_date, 
                    end_date=end_date,
                    aggregations={"Cost": "SUM", "Views": "SUM", "Clicks": "SUM", "Impression": "SUM"},
                    group_by=["Region", "Brand", "Funnel", "Platform", "Format", "Audience"]
                )
            except:
                # If the query fails, fallback to basic columns
                efficiency_df = query_data(
                    ["Region", "Cost", "Clicks", "Impression"], 
                    table, 
                    filters=active_filters, 
                    start_date=start_date, 
                    end_date=end_date,
                    aggregations={"Cost": "SUM", "Clicks": "SUM", "Impression": "SUM"},
                    group_by=["Region"]
                )
                ...
                if not efficiency_df.empty:
                    ...
                    # Reorder columns to match the requested structure
                    efficiency_df = efficiency_df[["Region", "Brand", "Funnel", "Platform", "Format", "Audience", "Cost", "Views", "Clicks", "Impression"]]
            ...
            if not efficiency_df.empty:
                ...
                efficiency_df['Clicks'] = pd.to_numeric(efficiency_df['Clicks'], errors='coerce').fillna(0)
                efficiency_df['Impression'] = pd.to_numeric(efficiency_df['Impression'], errors='coerce').fillna(0)
                
                # Calculate CPM, CPV, CPC with safe division
                efficiency_df['CPM'] = np.where(
                    efficiency_df['Impression'] > 0,
                    (efficiency_df['Cost'] / efficiency_df['Impression'] * 1000).round(0),
                    0
                )
                efficiency_df['CPV'] = np.where(
                    efficiency_df['Views'] > 0,
                    (efficiency_df['Cost'] / efficiency_df['Views']).round(0),
                    0
                )
                efficiency_df['CPC'] = np.where(
                    efficiency_df['Clicks'] > 0,
                    (efficiency_df['Cost'] / efficiency_df['Clicks']).round(0),
                    0
                )
                
                # Sort by CPM ascending (lower is better)
                efficiency_df = efficiency_df.sort_values('CPM', ascending=True)
                
                # Display Efficiency table using Material-UI style
                            ...
                            with mui.Paper(
                                ...
                                with mui.TableContainer(sx={"maxHeight": 400, "position": "relative"}):
                                    with mui.Table(stickyHeader=True, size="small"):
                                        ...
                                        with mui.TableHead():
                                            with mui.TableRow():
                                        headers = ["Region", "Brand", "Funnel", "Platform", "Format", "Audience", "CPM", "CPV", "CPC"]
                                                for header in headers:
                                                    mui.TableCell(
                                                        header,
                                                        sx={
                                    ...
                                    for _, row in efficiency_df.iterrows():
                                            ...
                                            # CPV
                                                    mui.TableCell(
                                                f"{row['CPV']:,.0f} ₫" if row['CPV'] > 0 else "-",
                                                sx={
                                                    "textAlign": "center",
                                                    "padding": "6px 4px",
                                                    "fontSize": "0.7rem",
                                            ...
                                            # CPC
                                            mui.TableCell(
                                                f"{row['CPC']:,.0f} ₫" if row['CPC'] > 0 else "-",
                                                        sx={
                                                            "textAlign": "center",
                                                            "padding": "6px 4px",
                                                            "fontSize": "0.7rem",
                                        ...
                                        # Calculate totals for sticky footer
                                total_cpm = efficiency_df['CPM'].mean()
                                total_cpv = efficiency_df['CPV'].mean()
                                total_cpc = efficiency_df['CPC'].mean()
                                        ...
                                        # Average CPV
                                                mui.TableCell(
                                            f"{total_cpv:,.0f} ₫" if total_cpv > 0 else "-",
                                                    sx={
                                                        "textAlign": "center",
                                                        "padding": "8px 6px",
                                                        "fontSize": "0.75rem",
                                        ...
                                        # Average CPC
                                                mui.TableCell(
                                            f"{total_cpc:,.0f} ₫" if total_cpc > 0 else "-",
                                                    sx={
                                                        "textAlign": "center",
                                                        "padding": "8px 6px",
                                                        "fontSize": "0.75rem",
                ...
                st.warning("No efficiency data available for the selected filters.")
