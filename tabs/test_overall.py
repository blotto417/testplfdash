...
def display_run_rate_circle(run_rate, active_days, plan_days):
    progress_percentage = run_rate * 100
    st.markdown(f"""
        <div class="kpi-container">
            <div class="kpi-title">🏃‍♂️ Run Rate</div>
            <div class="progress-circle" style="background: conic-gradient(var(--accent-blue) {progress_percentage}%, #e5e7eb 0% 100%);">
                <div class="progress-circle-inner">{run_rate:.1%}</div>
            </div>
            <div class="kpi-delta">{active_days} of {plan_days} days</div>
        </div>
    """, unsafe_allow_html=True)

def display_spent_bar(spent, planned_spend):
    spent_percentage = (spent / planned_spend * 100) if planned_spend > 0 else 0
    st.markdown(f"""
        <div class="kpi-container">
            <div class="kpi-title">💰 Spent</div>
            <div class="spent-bar-text">${spent:,.0f}</div>
            <div class="spent-bar-container">
                <div class="spent-bar" style="width: {spent_percentage}%;"></div>
            </div>
            <div class="kpi-delta">Budget: ${planned_spend:,.0f}</div>
        </div>
    """, unsafe_allow_html=True)

def display_progress_gauge(progress_diff):
    # Normalize progress_diff to a 0-180 degree scale
    # Assuming a range of -0.5 to 0.5 for progress_diff
    gauge_value = (progress_diff + 0.5) * 180
    gauge_value = max(0, min(180, gauge_value)) # Clamp between 0 and 180
    ...
    st.markdown(f"""
        <div class="kpi-container">
            ...
            <div class="gauge-container">
                ...
                <div class="gauge-needle" style="transform: translateX(-50%) rotate({gauge_value}deg);"></div>
            </div>
            <div class="gauge-value">{progress_diff:+.2f} pts</div>
            <div class="kpi-delta">Run Rate vs. Spending</div>
        </div>
    """, unsafe_allow_html=True)
