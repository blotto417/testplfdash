...
def display(query_data, active_filters, start_date, end_date):
    with stylable_container(
        ...
        if not aggrid_df.empty:
            ...
            # Configure columns with standardized formatting and center alignment
            gb.configure_column("Format", headerName="Format", cellStyle={'textAlign': 'center'})
            gb.configure_column("Creative_Type", headerName="Creative Type", cellStyle={'textAlign': 'center'})
            gb.configure_column("Creative_Length", headerName="Creative Length", cellStyle={'textAlign': 'center'})
            gb.configure_column("Platform", headerName="Platform", cellStyle={'textAlign': 'center'})
            gb.configure_column("Region", headerName="Region", cellStyle={'textAlign': 'center'})
            gb.configure_column("Audience", headerName="Audience", cellStyle={'textAlign': 'center'})
            gb.configure_column("Cost", type=["numericColumn", "numberColumnFilter", "customNumericFormat"], valueFormatter="data.Cost.toLocaleString('en-US')", cellStyle={'textAlign': 'center'})
            gb.configure_column("Impression", type=["numericColumn", "numberColumnFilter", "customNumericFormat"], valueFormatter="data.Impression.toLocaleString('en-US')", cellStyle={'textAlign': 'center'})
            gb.configure_column("Clicks", type=["numericColumn", "numberColumnFilter", "customNumericFormat"], valueFormatter="data.Clicks.toLocaleString('en-US')", cellStyle={'textAlign': 'center'})
            gb.configure_column("CTR", headerName="CTR", valueFormatter="(data.CTR * 100).toFixed(2) + '%'", cellStyle=ctr_style_jscode)
            gb.configure_column("ctr_bm", headerName="CTR Benchmark", type=["numericColumn", "numberColumnFilter", "customNumericFormat"], valueFormatter="(data.ctr_bm * 100).toFixed(2) + '%'", cellStyle={'textAlign': 'center'})
